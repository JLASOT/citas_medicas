...
export class SpecialitieService {
  ...
  updateSpecialitie(id: number, data: any): Observable<any> {
    const headers = new HttpHeaders({ token: this.authService.token });
    const URL = `${URL_SERVICIOS}/specialitie/${id}`;
    return this.http
      .put(URL, data, { headers })
      .pipe(catchError(this.handleError)); // Agregar manejo de errores
  }

  getSpecialitieById(id: string): Observable<any> {
    const headers = new HttpHeaders({ token: this.authService.token });
    const URL = `${URL_SERVICIOS}/specialitie/${id}`;
    return this.http
      .get(URL, { headers })
  ...
  deleteSpecialitie(id: number): Observable<any> {
    const headers = new HttpHeaders({ token: this.authService.token });
    const URL = `${URL_SERVICIOS}/specialitie/${id}`;
    return this.http
      .delete(URL, { headers })
